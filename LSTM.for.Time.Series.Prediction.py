...
data = pd.Series(np.sin(x) + stats.norm.rvs(loc=0, scale=0.02, size=1000), index=range(len(x)))
# f, ax = plt.subplots()
# util.plot_series(truth=data, ax=ax, title='sin')
# plt.show()

# >>>>>>>>>>>>  Split data  <<<<<<<<<<<<<< #
dgr = dataGenerator(data=data, lb=20, normalize_window=None, train_val_split=0.75, test_windows=200)
...
lstm = LSTM_model(input_shape=(20,1), structure='3-layers-lstm')
...
pred_test = dgr.inverse_transform(lstm.predict_multi_steps(X_test, steps=200), base_test)
print(pred_test)
# f, ax = plt.subplots()
